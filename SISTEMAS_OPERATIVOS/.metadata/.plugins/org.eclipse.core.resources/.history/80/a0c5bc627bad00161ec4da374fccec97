...
void copyNode(NodeStruct *dest, NodeStruct *src) {
	int i;
	dest->numEnlaces = src->numEnlaces;
...
int myMkfs(MyFileSystem *myFileSystem, int diskSize, char *backupFileName) {
	...
	int numBlocks = diskSize / BLOCK_SIZE_BYTES; // 2097152/ 4096 = 512 bloques
	int minNumBlocks = 3 + MAX_BLOCKS_WITH_NODES + 1; // 3+5+1 = 9
	int maxNumBlocks = NUM_BITS;// 4096
	...
	// Initialization
	int i;
	for(i = 0; i < NUM_BITS; i++) { // todos los bits del Mapa a 0
	...
	/// INODES
	NodeStruct currentNode;//creamos un nodo con info comun
	currentNode.freeNode = 1;// cada nodo estra libre
	for(i = 0; i < MAX_NODES; i++) {//entramos en cada nodo y lo actualizamos

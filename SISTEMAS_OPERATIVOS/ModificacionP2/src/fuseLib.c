...
int resizeNode(uint64_t idxNode, size_t newSize) {
	...
	/// File decreases
		...
		for(i = node->numBlocks; i > numBlocks; i--) {
			int nBloque = node->blocks[i - 1]; // indices de los bloques
		...
		node->numBlocks = numBlocks; // 0 si le pasamos en newSize 0
		node->fileSize += diff;
	}
...
static int my_link(const char *fichero,const char *enlace ){

	fprintf(stderr,"-----> my_link, fichero %s enlace %s\n", fichero, enlace);
	int idxFile = findFileByName(&myFileSystem, (char*)fichero+1);//buscamos en el SF nuestro nodo


}
static int my_unlink(const char *path){
	fprintf(stderr,"-----> my_unlink, path %s\n", path);
	int idxFile = findFileByName(&myFileSystem, (char*)path+1); //buscamos en el sistema y
	//y devolvemos la posicion en la que se encuentra en el DIRECTORIO dentro del FILESYSTEM



	/*if(idxFile== -1) // si es -1 no se encontró
		return -ENOENT; //fichero no encontrado
*/
	int idxNode  = myFileSystem.directory.files[idxFile].nodeIdx;// idxnode toma el valor del nodo asociado a ese fichero
	if(resizeNode(idxNode, 0) < 0)
		return -EIO;



	//SUPERBLOQUE: SABIENDO CUANTOS BLOQUES OCUPABA EL FICHERO, LO SUMAMOS A NUMERO DE BLOQUES LIBRES
	myFileSystem.superBlock.numOfFreeBlocks += myFileSystem.nodes[idxNode]->numBlocks;// sumamos bloques "borrados" al numero
	//de bloques libres en nuestro sistema.

	//NODOS
	myFileSystem.numFreeNodes++;
	myFileSystem.nodes[idxNode]->freeNode = 1;

	//BITMAP: CADA NODO ASOCIADO A UN FICHERO ES LIBERADO EN EL BITMAP
	int i,tmp;
	for(i = 0; i < MAX_BLOCKS_PER_FILE && myFileSystem.nodes[idxNode]->blocks[i] != 0; i++){

		//leemos los indices asociados al nodo
		tmp = myFileSystem.nodes[idxNode]->blocks[i];// cada uno tiene la posicion asociada en bitmap
		myFileSystem.bitMap[tmp] = 0;

	}

	//DIRECTORIO: ACTUALIZAMOS NUMERO DE FILES EN DIR Y EN LA POSICION DONDE ESTA EL FILE LO PONEMOS LIBRE.
	myFileSystem.directory.files[idxFile].freeFile = 1; // indicamos que esta libre
	myFileSystem.directory.numFiles--; //quitamos un archivo
...
static int my_read(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	char buffer[BLOCK_SIZE_BYTES];
	int bytes2Read, totalWrite = 0;
	NodeStruct *node = myFileSystem.nodes[fi->fh];

	fprintf(stderr, "--->>>MY_READ: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

	bytes2Read = node->fileSize;
	//bytes2Read = node->filseSize ;
	//char buffer[node->fileSize];
	int leidosTotal = 0;
	while(bytes2Read) {
		int i,leidos = 0;
		...
		for(i = offBlock; (i < BLOCK_SIZE_BYTES ) && (leidos+leidosTotal < node->fileSize ); i++) {
			 leidos++;

		}
		leidosTotal += leidos;
		memcpy(buf+ offset, buffer , BLOCK_SIZE_BYTES);
